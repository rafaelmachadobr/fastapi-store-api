...
from src.store.schemas.product import ProductOut, ProductUpdateOut
...
async def test_usecases_get_should_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.get(id=UUID("1e4f215e-85f7-461a-89d0-a751a32e3bb9"))

    assert (
        err.value.message
        == "Product not found with filter: 1e4f215e-85f7-461a-89d0-a751a32e3bb9"
    )
...
async def test_usecases_delete_should_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.delete(id=UUID("1e4f215e-85f7-461a-89d0-a751a32e3bb9"))
